...
class FragmentsHttp4kAdapterTest {
    ...
    class StubTemplateRenderer : (Any) -> String {
        override fun invoke(viewModel: Any): String = "Stub template"
    }

    @Test
    fun homeEndpointReturns200() {
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/")
                .execute(server)
    ...
    fun nonExistentPageReturns404() {
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/page/non-existent")
                .execute(server)
    ...
    fun existingPageReturns200() {
        ...
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/page/test-page")
                .execute(server)
        assertEquals(Status.OK, response.status)
    }
    ...
    fun blogOverviewReturns200() {
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/blog")
                .execute(server)
        assertEquals(Status.OK, response.status)
    }
    ...
    fun blogPostWithValidPathReturns404WhenMissing() {
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/blog/2024/01/test-post")
                .execute(server)
        assertEquals(Status.NOT_FOUND, response.status)
    }
    ...
    fun blogPaginationReturns200() {
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/blog/page/2")
                .execute(server)
        assertEquals(Status.OK, response.status)
    }
    ...
    fun blogTagReturns200() {
        ...
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/blog/tag/kotlin")
                .execute(server)
        assertEquals(Status.OK, response.status)
    }
    ...
    fun blogCategoryReturns200() {
        ...
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/blog/category/tech")
                .execute(server)
        assertEquals(Status.OK, response.status)
    }
    ...
    fun rssFeedReturns200() {
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/rss.xml")
                .execute(server)
        assertEquals(Status.OK, response.status)
    ...
    fun robotsTxtReturns200WithCorrectContent() {
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/robots.txt")
                .execute(server)
        assertEquals(Status.OK, response.status)
    ...
    fun llmsTxtReturns200WithCorrectContent() {
        ...
        val response =
            Request(Method.GET, "http://localhost:${server.port()}/llms.txt")
                .execute(server)
        assertEquals(Status.OK, response.status)
        assertEquals("text/plain; charset=utf-8", response.header("Content-Type"))
        val body = response.bodyString()
...
private fun Request.execute(server: Http4kServer): Response = org.http4k.client.OkHttp()(this)
